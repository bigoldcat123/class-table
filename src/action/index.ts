'use server'
import { z } from 'zod'
import { TableCellProp } from "@/components/tableCell"
import db from "@/db"
...
export async function getClassTable(date: Date) {
    ...
    const tempClasses = await db.select().from(tempClassTable).where(and(gte(tempClassTable.execDate, m_s.monday), lte(tempClassTable.execDate, m_s.sunday)))

    const classMarks = await db.select().from(classMark).where(and(gte(classMark.finishDate, m_s.monday), lte(classMark.finishDate, m_s.sunday)))
    const map = new Map<string, TableCellProp>()
    classes.forEach(x => {
        map.set(`${x.week}${x.order}`, { classAndRoom: x.class_and_room, name: x.name, isTemp: false, isFinished: false })
    })
    // console.log(tempClasses);

    tempClasses.forEach(x => {
        if (x.type) {// add
            map.set(`${x.week}${x.order}`, { classAndRoom: x.class_and_room, name: x.name, isTemp: true, isFinished: false })
        } else {
...
export async function addTempAction(stat: string, formData: FormData) {
    ...
    await db.insert(tempClassTable).values(temp as typeof tempClassTable.$inferSelect)
    revalidatePath("/", 'page')
    return stat + "ok!"
}

export async function getTempAction() {
    const temps = await db.select().from(tempClassTable).orderBy(desc(tempClassTable.execDate))
    return temps
}
...
export async function markAsFinishedAction({ clazz, coordinate }: {
    coordinate: {
        week: number,
        order: number
    },
...
export async function getJobMarkAction(query: { name?: string, startDate?: string, endDate?: string } = { name: undefined, startDate: undefined, endDate: undefined }) {
    ...
    const marks = await db.select().from(classMark).orderBy(desc(classMark.finishDate)).where(
        and(
            queryParsed.name ? like(classMark.name, `%${queryParsed.name}%`) : undefined,
            queryParsed.startDate ? gte(classMark.finishDate, queryParsed.startDate) : undefined,
            queryParsed.endDate ? lte(classMark.finishDate, queryParsed.endDate) : undefined
        )
    );
    return marks;
}